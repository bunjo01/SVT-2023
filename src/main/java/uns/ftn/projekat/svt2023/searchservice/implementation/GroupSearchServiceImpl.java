...
public class GroupSearchServiceImpl implements GroupSearchService {
    ...
    private static final Logger log = LoggerFactory.getLogger(GroupSearchServiceImpl.class);
    private final ElasticsearchOperations elasticsearchTemplate;

    @Override
    public Page<GroupIndex> groupSimpleSearch(List<String> keywords, Pageable pageable) {
    ...
    @Override
    public Page<GroupIndex> groupAdvancedSearch(List<String> expression, Pageable pageable) {
    ...
    private Query buildSimpleSearchQuery(List<String> tokens) {
        return BoolQuery.of(q -> q.must(mb -> mb.bool(b -> {
            tokens.forEach(token -> {
                b.should(sb -> sb.match(m -> m.field("name").fuzziness(Fuzziness.ONE.asString()).query(token).analyzer("serbian")));
                b.should(sb -> sb.match(m -> m.field("description").query(token).analyzer("serbian")));
                b.should(sb -> sb.match(m -> m.field("pdf_text").query(token).analyzer("serbian")));  // Change made here
            });
            return b;
        })))._toQuery();
    }
    ...
    private Page<GroupIndex> runQuery(NativeQuery searchQuery) {
        ...
        var searchHits = elasticsearchTemplate.search(searchQuery, GroupIndex.class, IndexCoordinates.of("groups"));
